...
public class TramHunterDB extends SQLiteOpenHelper {
	...
	private static final int DATABASE_VERSION = 1391;
	...
	/**
	 * Get a database object from either internal or external storage
	 * 
	...
	private SQLiteDatabase getInternalStorageDB() {
		...
		String dbFile = mContext.getDatabasePath(DATABASE_NAME).getAbsolutePath();
		if (LOGV) Log.w(TAG, "Using DB path: " + dbFile);
	 ...
	 * 
	 * @param dbFile
	 *            - a String representing the absolute file name
	 * @throws IOException
	 */
	...
	public synchronized void close() {
		if (mDB != null) if (mDB.isOpen()) mDB.close();
	...
	public Route getRoute(int routeId) {
		...
		if (c.moveToFirst()) {
			...
			int col_internal_number = c.getColumnIndexOrThrow(Routes.INTERNAL_NUMBER);

			route.setId(routeId);
			route.setNumber(c.getString(col_number));
			route.setInternalNumber(c.getInt(col_internal_number));
	...
	/**
	 * Get a destination from a given destination ID
	 * 
	 * @return Destination
	 */
	...
	public List<Stop> getAllStops() {
		...
		Cursor c = db.query(TABLE_STOPS, null, null, null, null, null, null, null);
	...
	public List<Stop> getStopsForSearch(String searchString) {
		...
		String searchQuery = StopsColumns.FLAG_NUMBER + " LIKE '%" + searchString + "%' OR " + StopsColumns.PRIMARY_NAME + " LIKE '%" + searchString + "%'"
				+ "OR " + StopsColumns.SECONDARY_NAME + " LIKE '%" + searchString + "%'" + "OR " + StopsColumns.TRAMTRACKER_ID + " LIKE '%" + searchString
				+ "%'";
		...
		Cursor c = db.query(TABLE_STOPS, null, searchQuery, null, null, null, null, searchLimit);
	...
	/**
	 * Build a stop object from a given cursor
	 * 
	 * @param c
	 * @return Stop
	 */
